...
使用 curl 发送请求到 Gemini API，**必须使用 `-s` 静默模式**避免进度信息混入输出：
...
**重要**: 使用 `-s` (silent) 参数确保输出纯净 JSON，不包含 curl 进度信息。
...
- **JSON 验证失败**: 如果响应文件不是有效 JSON，尝试用 `tail -n +20` 跳过 curl 进度信息提取有效内容，而非直接重新调用 API
- **响应无图片**: 提示用户响应中未包含图片
- **文件写入失败**: 检查目录权限，报告错误
...
1. **避免重复调用 API**: API 调用有成本（时间、token、费率），遇到问题先尝试修复现有数据
2. **使用 `-s` 静默模式**: curl 必须加 `-s` 参数，确保输出纯净 JSON
3. **验证后再处理**: 保存响应后立即验证 JSON 有效性，避免将错误传递给后续步骤
