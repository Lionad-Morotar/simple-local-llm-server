...
2.1 评估任务难度，获得变量 `$mode`（三选一：easy、medium、hard，默认 easy）
2.2 选择初始研究方向，是拓宽搜索范围，还是先深入研究特定领域
2.3 你可以使用以下或更多技巧：
  - 搜索技巧：并排比较、时间限制等
  - 公开基准：具名评估数据集/框架等
...
根据 `$mode`，**并行启动 2~4 个代理（easy 2 个代理、medium 3、hard 4）**，每个代理使用不同的查询集群。
...
1. 根据评估其建议拓宽搜索范围、或深入特定领域研究。
2. 根据评估器的建议启动下一轮搜索或完成研究。
3. 更新 todos，并向用户简单汇报此轮的评估结果和改进建议。
...
在会话中，向用户返回一个十分精简的研究摘要。然后撰写一个更详细的报告。

报告（即最终摘要）仿照以下格式（不做强制要求），写入 `/tmp/search-web/{YYYY-MM-DD}/{task-name}/report.md`。
