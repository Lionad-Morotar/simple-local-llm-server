...
import { processMd } from "./mdProcessor";
...
async function batchConvert(): Promise<void> {
  ensureDirectoryExists(TXT_DIR);
  ensureDirectoryExists(PROCESSED_DIR);

  const mdFiles = getMdFiles(MD_DIR);
  const total = mdFiles.length;

  if (total === 0) {
    console.log("📂 未找到任何 Markdown 文件");
    console.log(`💡 请将 .md 文件放在 ${MD_DIR} 目录下`);
    return;
  }

  // 过滤已转换的文件和出错的文件
  const filesToConvert = mdFiles.filter((filename) => {
  ...
  const processNext = async (): Promise<void> => {
    while (currentIndex < filesToConvert.length) {
      ...
      const tempTextPath = path.join(PROCESSED_DIR, `${filename}.temp.txt`);

      activeWorkers++;

      try {
        console.log(`🔄 [${completed + 1}/${toConvertCount}] 处理 Markdown: ${filename}`);

        // 第一步：处理 Markdown 为临时文本
        const mdResult = await processMd(mdPath, tempTextPath);

        if (!mdResult.success) {
          throw mdResult.error || new Error("Markdown 处理失败");
        }

        // 第二步：Worker 处理文本
        const workerResult = await processInWorker(tempTextPath, txtPath);

        completed++;

        if (workerResult.success) {
          console.log(`✅ [${completed}/${toConvertCount}] ${filename} → ${txtFilename}`);
          doneManager.addDone(filename);
          ...
          throw workerResult.error || new Error("Worker 处理失败");
        }
      } catch (error) {
        completed++;
        const errorMessage = error instanceof Error ? error.message : String(error);
        console.error(
          `❌ [${completed}/${toConvertCount}] ${filename} 转换失败: ${errorMessage}`
        );
        errorManager.addError(filename, error instanceof Error ? error : new Error(String(error)));
